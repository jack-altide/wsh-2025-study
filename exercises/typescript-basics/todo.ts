...
const task1: { id: number; title: string; completed: boolean } = {
  id: 1,
  title: "TypeScriptを学ぶ",
  completed: false,
};
...
const task2: { id: number; title: string; completed: boolean } = {
  id: 2,
  title: "Node.jsを学ぶ",
  completed: true
};
...
// 型エイリアス: 型に名前をつける
// これで「{ id: number; title: string; completed: boolean }」を
// 「Task」という短い名前で使える
type Task = {
  id: number;
  title: string;
...
// ユニオン型: 「AまたはBまたはC」を表す
// "pending" | "in_progress" | "done" の3つだけが許可される
...
type TaskV2 = {
  id: number;
  title: string;
  status: Status;  // boolean → ユニオン型に変更
};

const tasksV2: TaskV2[] = [
...
function filterByStatus(taskList: TaskV2[], status: Status): TaskV2[] {
  return taskList.filter((task) => task.status === status);
}

// タスクを追加する関数
// 戻り値がない場合は void を使う
function addTask(taskList: TaskV2[], title: string): void {
  const newId = taskList.length + 1;
  taskList.push({ id: newId, title: title, status: "pending" });
}

// タスクのステータスを変更する関数
function updateStatus(taskList: TaskV2[], id: number, newStatus: Status): void {
...
// インターセクション型: TaskV2 & WithPriority
// → TaskV2の全プロパティ + priorityプロパティ を持つ型
type PriorityTask = TaskV2 & WithPriority;
